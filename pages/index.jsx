...
export async function getServerSideProps() {
  ...
  const [datacommingsoon, datatrendMovies, datatrendTvShows] = await Promise.all([
    await new Promise(resolve => setTimeout(() => resolve(getCommingSoonMovie()), 1500)),
    await new Promise(resolve => setTimeout(() => resolve(getPapularMovies()), 1500)),
    await new Promise(resolve => setTimeout(() => resolve(getPapularTvShows()), 1500)),
  ]);
