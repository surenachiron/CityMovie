...
export async function getServerSideProps({ params }) {
    ...
    const [datamovie, photosmovie, castsmovie] = await Promise.all([
        await new Promise(resolve => setTimeout(() => resolve(getDetailsMovie(params.id)), 2000)),
        await new Promise(resolve => setTimeout(() => resolve(getPhotos(params.id)), 4000)),
        await new Promise(resolve => setTimeout(() => resolve(getCasts(params.id)), 6000))
    ]);
    return {
        props: {
            movie: datamovie,
