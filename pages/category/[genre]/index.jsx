...
const SingleCategory = ({ categoryMovie }) => {
  ...
  if (categoryMovie === null || categoryMovie.length === 0)
    return <CustomError />;

  const router = useRouter();
  const genreSelected =
    router.query.genre.charAt(0).toUpperCase() + router.query.genre.slice(1);
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={`show trend movie from ${genreSelected}`}
        />
      </Head>
      <div>
...
export async function getServerSideProps({ params }) {
  const dataCategoryMovie = await getCategoryMovies(params.genre);
